...
## Palette sample, not yet applied

`sample-home.html` is the live home page with one extra stylesheet,
`assets/css/palette-sample.css`, taking the surfaces and ink from the Claude
Design system doc v1.0 and keeping the site's own terracotta orange as the
accent. Cream papers, indigo ink, orange buttons.

It is generated by `python sample_palette.py`, which reads `index.html` and adds
the stylesheet, a `noindex`, a new title and the three inline chrome colours the
markup carries. So the sample is the real page rather than a copy of it: re-run
it after `build.py` and it follows along. Verified by measuring both pages side
by side at 1345px — every box on the same pixel, same document height.
