...
Surfaces and ink from the Claude Design system doc v1.0, keeping the site's own
terracotta orange as the accent. Cream papers, indigo ink, orange buttons. It now
runs on all six pages.

It was developed as an override stylesheet against the home page only —
`sample-home.html` plus `assets/css/palette-sample.css`, generated by
`sample_palette.py` — so that the live site stayed untouched while it was being
argued over. Those three files are gone; git history has them if the reasoning
behind a value is ever wanted in its original form.

Three things the reference cannot have as written, each noted at the token:

- Its contrast figures are computed against white, not against the paper they sit
  on. `ink-faint` #7E819B is quoted at 3.6:1 but is 3.45:1 on fresh paper and
  2.74:1 on dried, and it is specified for 13-15px notes. `--ink-3` uses
  `ink-muted` #4E5372 instead, 6.77:1 and 5.37:1 on dried.
- Its accent #C4522C is 4.14:1, so it cannot carry text at AA either. Moot here,
  because the accent stays the site's own orange.
- It names one rule colour where this layout uses three line weights, so
  `--sand-400` and `--sand-500` continue its ramp in the direction it was already
  going. Derived, and marked as derived.

Promoting it found one thing the sample could not have. Form fields were
`--sand-100`, which under the old palette was #FEF6F0 against a #FFFCFA
`--sand-50` — indistinguishable, so nobody had noticed the wrong token. The new
palette makes `--sand-50` pure white and `--sand-100` the warm page, which left
every field on `start.html` the same colour as the paper behind it with a 1px
border to argue otherwise. The sample only ever covered the home page, which has
no form on it.

The `REFINEMENTS` half of `style.css` is not colour. It carries the design
changes made on top of the palette, and it is deliberately a layer: it restates
some of the rules above it rather than editing them in place, because most of
those originals are still the right default for a section that has not been
reworked, and because each block is only legible as a correction to the one it
replaces. It holds: hairline-separated three-up grids with capped measures instead of three
...
The dot is a flat 10px disc now, one value per state, with no ring at all: it was
two-tone in both states, and taking the lit centre off the accent while leaving
the border on it produced an orange ring around an olive core. The paper halo is
what interrupts the line, not the contrast, which is 1.30 between a passed bead
and the fill on purpose.
...
`assets/js/ride.js` is the one piece of this that is not in the stylesheet, and
the only page that gets it is the home page: it is the hero's button compacted, so
it needs one to start from, and `shell()` ships the script by asking the markup for
`class="hero-cta"` rather than keeping a list of slugs.

Everything in CSS that hides a real call to action in favour of the ride is gated
on `html[data-ride]`, which the script sets only after every early return. That
gate is load-bearing rather than tidy: `style.css` already hides `.close .act`
above 1180px because the disc at the foot of the rail is the call to action there,
so the ungated rules would have left the five pages without a hero button with no
call to action at all at desktop width. Gated, a page without a ride keeps the
foot disc exactly as it always had it.

The closing button's label is a `shell()` parameter for the same reason. On a page
where the ride runs, the button the reader ends up with is the hero's button
carried down, so the two have to read the same or the words change in the instant
the handover exists to hide. Past the hero the call to action compacts to a dot on the rail and
...
the same size in the same place. What the reader ends up with is a link inside a section,
...
Below 1180px, and under reduced motion, none of it is built, `data-ride` is never
set, and the page keeps the call to action it would have had anyway.
